...
<script type="text/javascript" src="static/js/libs/jquery.loadmask.min.js"></script>
</head>
<script type="text/javascript" >
	...
	function openSkill(){
		$.webox({
			...
			bgvisibel:true,
			title:'快捷下单',
			iframe:'<%=basePath%>seckillAction/openSeckillOrder'
		});
	}
	
	function openSkillList(){
		var url = "<%=basePath%>seckillAction/openList";
		var vcStart = $("#vcStart").val();
		if(null!=vcStart && ""!=vcStart){
			url = url + "?vcStart="+encodeURI(encodeURI(vcStart));
			var vcEnd = $("#vcEnd").val();
			if(null!=vcEnd && "" !=vcEnd){
				url = url+"&vcEnd="+encodeURI(encodeURI(vcEnd));
			}
		}else{
			var vcEnd = $("#vcEnd").val();
			if(null!=vcEnd && "" !=vcEnd){
            ...
            <div class="block">
                ...
                <div class="block_con" id="login">
                	...
                	<form action="j_spring_security_check" method="post">
                    	<div><input type="text" class="f_txt" placeholder="用户名/email" name="account"  id="account"/></div>
                        <div><input type="password" class="f_txt" placeholder="密码" name="password" id="password" /></div>
...
<table width="100%" border="0" class="table_form">
  ...
  <tr>
    <td>&nbsp;</td>
    <td><input name="" type="submit" value="" class="f_search2" onclick="openSkillList();" /></td>
  </tr>
</table>
