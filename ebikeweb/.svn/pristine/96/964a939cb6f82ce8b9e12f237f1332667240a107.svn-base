...
$(document).ready(function(){
	...
	var size = getPageSize(h);
		$('#dg').datagrid({     
		    url:'<%=basePath%>basicManage/getDriverClassList',
			cache : false,
			type : 'post',
			title : '司机分类',
		...
		var selected = $('#dg').datagrid('getSelected');
		 if (selected){
		       var index = $('#dg').datagrid('getRowIndex', selected);
		     	...
		     	var url = "<%=basePath%>basicManage/saveDirverClassBefore?cuID="+selected.id;
				$.webox({
						height:580,
						width:800,
						bgvisibel:true,
	...
	function addDriverTag(){
		var url = "<%=basePath%>basicManage/saveDirverClassBefore";
		$.webox({
				height:580,
				width:800,
				bgvisibel:true,
