...
$(document).ready(function() {
		...
		if(paramsType=="add")
		{
			$("#myform").attr("action","<%=basePath%>basicManage/saveCarStylePic");
		...
		$("#myform").validate({
			 ...
			 messages:{
			  ...
			  submitHandler:function(form){
				 ...
				 ({
					        type:"post",
					        url:"<%=basePath%>basicManage/saveCarStylePic",
...
<body class="main">
	<form action="<%=basePath%>basicManage/saveCarStylePic" enctype="multipart/form-data"
		method="post" id="myform">
		<dl class="tabs">
			<dt>
				<a href="#"><span>汽车商标维护</span> </a>
			</dt>
			<dd class="form">
				<input type="hidden" name="id" value="${tCarStylePic.id }" />
				<table width="100%" border="0" class="table_form">
					<tr>
						<td>车型</td>
						<td><input class="f_txt" name="vcStyle"
							value="${tCarStylePic.vcStyle}" type="text" id="vcStyle" />
						</td>
						<th>是否有效：</th>
						<td><input name="nEnable" type="radio" value="0"
							...
							name="NEnabletxt" value="${tCarStylePic.nEnable }" /> <script
					...
					<tr>
						<td>状态图标</td>
						<td>
							<input type="hidden" name="vcPath" value="${tCarStylePic.vcPath}"/>
							<c:if test="${tCarStylePic.vcPath != null}">
							<img src="<%=basePath%>static/carstyle_pic/${tCarStylePic.vcPath}" alt="" />
							</c:if>
						</td>
