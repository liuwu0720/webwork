...
$(document).ready(function(){
	...
	var size = getPageSize(h);
		$('#dg').datagrid({     
		    url:'<%=basePath%>basicManage/getPicTypeList',
			cache : false,
			type : 'post',
			title : '图片分类管理',
		...
		var selected = $('#dg').datagrid('getSelected');
		 if (selected){
		       var index = $('#dg').datagrid('getRowIndex', selected);
		     	...
		     	var url = "<%=basePath%>basicManage/savePicTypeBefore?cuID="+selected.id;
				$.webox({
						height:580,
						width:800,
						bgvisibel:true,
	...
	//新增
	function add(){
		var url = "<%=basePath%>basicManage/savePicTypeBefore";
		$.webox({
				height:580,
				width:800,
				bgvisibel:true,
