...
$(document).ready(function(){
	...
	var size = getPageSize(h);
		$('#dg').datagrid({     
		    url:'<%=basePath%>basicManage/getUserStatusList',
			cache : false,
			type : 'post',
			title : '用户状态',
		...
		var selected = $('#dg').datagrid('getSelected');
		 if (selected){
		       var index = $('#dg').datagrid('getRowIndex', selected);
		     	...
		     	var url = "<%=basePath%>basicManage/saveUserStatusBefore?cuID="+selected.id;
				$.webox({
						height:580,
						width:800,
						bgvisibel:true,
	...
	//新增
	function add(){
		var url = "<%=basePath%>basicManage/saveUserStatusBefore";
		$.webox({
				height:580,
				width:800,
				bgvisibel:true,
