  ...
  <head>
    ...
    <script type="text/javascript">
    	...
    	function getCitys(id,value){
			...
			provinceId=id;
			provinceName=value;
			$('#addr1',window.parent.document).val(provinceName);//给输入框赋值
        ...
        function selectCity(cityId,cityName){
            ...
            //alert(addr);
			$('#addr1',window.parent.document).val(addr);//给输入框赋值
			$('#pro1',window.parent.document).val(provinceId);//给隐藏域省份赋值省
			$('#city1',window.parent.document).val(cityId);//给隐藏域城市赋值
