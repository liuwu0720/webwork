...
$(document).ready(function(){
	...
	var size = getPageSize(h);
		...
		$('#dg').datagrid({     
		    url:'<%=basePath %>integalCutAction/getAllIntegalCuts',
		    cache : false,
		    type : 'post',
		    title:'积分扣除类型信息', 
		    ...
		    columns:[[     
		        ...
		        {field:'vcCause',title:'扣除原因',width:100,align:'left'}, 
		        {field:'nintegal',title:'扣分数量',width:100,align:'right'}, 
		        {field:'vcCode',title:'扣分编码',width:100,align:'right'},
	...
	function save()
	{
		var url = "<%=basePath%>integalCutAction/saveBefore";
		$.webox({
				height:360,
				width:600,
				bgvisibel:true,
	...
	function del()
	{
	    ...
	    if (selected){
	       var index = $('#dg').datagrid('getRowIndex', selected);
	     	...
	     	$.post("<%=basePath%>integalCutAction/delIntegalCuts", 
					{ inteCutID:selected.id },     
		...
		var selected = $('#dg').datagrid('getSelected');
		 if (selected){
		       var index = $('#dg').datagrid('getRowIndex', selected);
		     	...
		     	var url = "<%=basePath%>integalCutAction/saveBefore?inteCutID="+selected.id;
				$.webox({
						height:360,
						width:600,
						bgvisibel:true,
	...
	function seacher(){
		...
		if(null!=vcCode && ""!= vcCode){
			jsonStr.vcCode=vcCode;
		}
		if(null!=vcCause && ""!= vcCause){
			jsonStr.vcCause=vcCause;
		}
...
<body class="main easyui-layout" onresize="resizeGrid(400);">
	<dl class="tabs">
    	<dt><a href="#"><span>扣分类型管理</span></a></dt>
        <dd class="form">
        <div class="batch">
        ...
        <div class="search">
			  ...
			  <input type="text" id="vcCause" class="f_txt f_short" placeholder="原因"/> 
			   <input type="text" id="vcCode" class="f_txt f_short" placeholder="编码"/>  
