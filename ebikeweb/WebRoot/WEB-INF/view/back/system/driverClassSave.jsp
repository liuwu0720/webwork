...
$(document).ready(function() {
		...
		if(paramsType=="add")
		{
			$("#myform").attr("action","<%=basePath%>basicManage/saveDirverClass");
		...
		$("#myform").validate({
			 ...
			 messages:{
			  ...
			  submitHandler:function(form){
				 ...
				 ({
					        type:"post",
					        url:"<%=basePath%>basicManage/saveDirverClass",
...
<body class="main">
	<form action="<%=basePath%>basicManage/saveDirverClass"
		method="post" id="myform">
		<dl class="tabs">
			<dt>
				<a href="#"><span>司机分类维护</span> </a>
			</dt>
			<dd class="form">
				<input type="hidden" name="id" value="${tDriverClass.id }" />
				<table width="100%" border="0" class="table_form">
					<tr>
						<td>司机标签</td>
						<td><input class="f_txt" name="vcTag"
							value="${tDriverClass.vcTag}" type="text" id="vcTag" />
						</td>
						<th>是否有效：</th>
						<td><input name="nEnable" type="radio" value="0"
							...
							name="NEnabletxt" value="${tDriverClass.nEnable }" /> <script
