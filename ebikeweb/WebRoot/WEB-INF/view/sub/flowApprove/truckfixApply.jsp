...
$(document).ready(function(){
	...
	var w = getWidth(400);
		$('#dg').datagrid({     
		    url:'<%=basePath%>fixTruckAction/getFixTrucklist',
		    cache : false,
		    type : 'post',
		    title:'修车申请审批', 
			...
			onExpandRow: function(index,row){
				...
				ddv.datagrid({
					url:'<%=basePath %>fixTruckAction/getPicDetail?id='+row.id,
					fitColumns:true,
					rownumbers:true,
					loadMsg:'正在加载...',
					height:500,
	...
	function sure(obj){
		...
		if(confirm("确认要审核通过所选记录吗？")){
			var vcNote = $("#note").val();
			$.post("<%=basePath%>fixTruckAction/sure", {
	...
	function serachMap(id){
		...
		tmp.focus()  
		tmp.location="fixTruckAction/getCurrentMap?id="+id; 
